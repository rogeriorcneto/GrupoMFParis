-- Fix: mover_cliente_atomico agora persiste status_follow_up em qualquer etapa.
-- Mantém o comportamento de limpar status_follow_up ao voltar para 'negociacao'
-- quando nenhum valor for enviado em p_extras.
...
BEGIN
  ...
  UPDATE clientes SET
    ...
    status_follow_up = CASE
      WHEN p_extras->>'status_follow_up' IS NOT NULL AND p_extras->>'status_follow_up' != '' THEN p_extras->>'status_follow_up'
      WHEN p_etapa = 'negociacao' THEN NULL
      ELSE status_follow_up
    END
  ...
  -- 4. Limpar campos ao voltar para prospecção
  ...
  -- 5. Inserir histórico de etapa (cast para timestamptz)
