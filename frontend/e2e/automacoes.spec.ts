...
test.describe('Automações de Vendas', () => {
  test('view renderiza sem erros para gerente', async ({ page }) => {
    ...
    await expect(page.getByText('Automações de Vendas')).toBeVisible({ timeout: 10_000 })
  ...
  test('gerente consegue ver cadências e regras', async ({ page }) => {
    ...
    // A view deve ter algum conteúdo (tabs, cards, listas)
    // Verifica que renderizou algo além do título
    const content = page.locator('main, [class*="content"], [class*="container"]').first()
