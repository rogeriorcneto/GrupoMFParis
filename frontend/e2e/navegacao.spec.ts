...
test.describe('Navegação completa — Gerente', () => {
  ...
  for (const v of views) {
    test(`gerente navega para: ${v.button}`, async ({ page }) => {
      ...
      // Usa regex exact match para evitar matches parciais (ex: "Integrações" vs "Outras Integrações")
      const titleRegex = new RegExp(`^${v.title.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}$`)
      await expect(page.locator('h2', { hasText: titleRegex })).toBeVisible({ timeout: 10_000 })
  ...
  test('gerente: Dashboard renderiza métricas', async ({ page }) => {
    ...
    await expect(page.getByText('Total Leads').first()).toBeVisible({ timeout: 10_000 })
  ...
  test('gerente: Funil renderiza colunas do pipeline', async ({ page }) => {
    ...
    await expect(page.getByText('Prospecção').first()).toBeVisible({ timeout: 10_000 })
    await expect(page.getByText('Amostra').first()).toBeVisible()
    await expect(page.getByText('Proposta').first()).toBeVisible()
    await expect(page.getByText('Negociação').first()).toBeVisible()
    await expect(page.getByText('Follow-up').first()).toBeVisible()
    await expect(page.getByText('Cliente Ativo').first()).toBeVisible()
  })
  ...
  test('gerente: Clientes renderiza tabela', async ({ page }) => {
    ...
    await expect(page.locator('h2', { hasText: 'Clientes' })).toBeVisible({ timeout: 10_000 })
  })
