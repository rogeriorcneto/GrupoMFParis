  ...
  const confirmPerda = () => {
    if (draggedItem) {
      moverCliente(draggedItem.cliente.id, 'perdido', {
        motivoPerda: motivoPerdaTexto || 'Não informado',
      ...
      {selectedClientePanel && (() => {
        ...
        return (
          <div className="fixed inset-0 z-40 flex justify-end">
            <div className="absolute inset-0 bg-black bg-opacity-30 hidden sm:block" onClick={() => setSelectedClientePanel(null)} />
            <div className="relative w-full sm:max-w-lg bg-white shadow-2xl overflow-y-auto animate-slide-in-right">
              <div className="sticky top-0 bg-white border-b border-gray-200 px-4 sm:px-6 py-4 flex items-center justify-between z-10">
                <div>
                  <h2 className="text-lg font-bold text-gray-900">{c.razaoSocial}</h2>
                  {c.nomeFantasia && <p className="text-sm text-gray-500">{c.nomeFantasia}</p>}
                </div>
                <button onClick={() => setSelectedClientePanel(null)} className="p-2 hover:bg-gray-100 rounded-apple"><XMarkIcon className="h-5 w-5 text-gray-500" /></button>
              </div>

              <div className="px-4 sm:px-6 py-5 space-y-5">
                {/* Badge etapa + score */}
                <div className="flex items-center gap-3">
                  <span className={`px-3 py-1 text-xs font-bold rounded-full ${etapaCores[c.etapa] || 'bg-gray-100 text-gray-800'}`}>{etapaLabels[c.etapa] || c.etapa}</span>
                  <span className="text-xs text-gray-500">Há {diasNaEtapa} dias nesta etapa</span>
                  {c.score !== undefined && (
                    <div className="ml-auto flex items-center gap-1">
                      <span className="text-xs font-bold text-gray-700">Score: {c.score}</span>
                      <div className="w-12 bg-gray-200 rounded-full h-1.5"><div className="bg-blue-600 h-1.5 rounded-full" style={{ width: `${c.score}%` }} /></div>
                    </div>
                  )}
                </div>
      ...
      {showMotivoPerda && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-apple shadow-apple-lg max-w-md w-full p-6">
            ...
            <textarea value={motivoPerdaTexto} onChange={(e) => setMotivoPerdaTexto(e.target.value)} rows={2} placeholder="Descreva o motivo..." className="w-full px-3 py-2 border border-gray-300 rounded-apple focus:outline-none focus:ring-2 focus:ring-primary-500 mb-4 text-sm resize-none" />
            <div className="flex justify-end gap-3">
              ...
              <button onClick={confirmPerda} className="px-4 py-2 bg-red-600 text-white rounded-apple hover:bg-red-700 text-sm font-medium">Confirmar Perda</button>
            </div>
          </div>
        </div>
      )}
